...
try{
	...
	int f=0,f1=0,f2=0;
	String atype="",type="billpay",y="",paye="";
    String accno=(String)session.getValue("ano"); 
        //String accno="4";
    String toaccno=request.getParameter("T1");
    String payfor=request.getParameter("D1");
  	String num=request.getParameter("D2");
	String payamt=request.getParameter("T4");
	int payamt1=Integer.parseInt(payamt);
       	Connection cc=null;
	...
	while(rs1.next())
	{
		...
		{	
			balance1=balance-payamt1;
		}		

if(check!=1)
{		

	String sq=("insert into billpay values(?,?,?,?,?)");                       			
    PreparedStatement p3=cc.prepareStatement(sq);
	p3.setInt(1,Integer.parseInt(accno));
	p3.setInt(2,Integer.parseInt(toaccno));
	p3.setInt(3,Integer.parseInt(num));
	p3.setInt(4,payamt1);                 
	p3.setString(5,x);                 
       	i=p3.executeUpdate();
	if(i>0)
...
String sq3=("insert into wdraw values(?,?,?,?,?,?,?,?,?,?,?,?,?,?)");
	...
	p4.setInt(9,n);
	p4.setString(10,y);
	p4.setString(11,toaccno);
	p4.setInt(12,payamt1);
